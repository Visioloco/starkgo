  ...
  Widget build(BuildContext context) {
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      TextFormField(
        ...
        style: GoogleFonts.spaceGrotesk(
            color: _C.textPri, fontSize: 14, fontWeight: FontWeight.w500),
        decoration: InputDecoration(
          hintText: hint,
          hintStyle: GoogleFonts.spaceGrotesk(
              color: _C.textSec.withOpacity(0.6), fontSize: 13),
...
class _RegistrarPagoWidgetState extends State<RegistrarPagoWidget> {
  ...
  Future<void> _enviarWhatsappEvolution({
    ...
    final String mensaje = _construirMensaje(
        nombre: nombre,
        referencia: referencia,
        fecha: fecha,
        valor: valor,
    ...
    try {
      final url = Uri.parse(
          '${instancia.serverUrl}/message/sendText/${instancia.instanceName}');
  ...
  Future<void> _registrarPago() async {
    ...
    final String metodoPago = _model.textController5!.text.trim().isEmpty
        ? 'Efectivo'
          ...
          builder: (_) => AlertDialog(
            ...
            content: Column(
                ...
                children: [
                  ...
                  _ConfirmRow(
                      label: 'Referencia', value: _model.textController1.text),
                  _ConfirmRow(
                      label: 'Valor',
                      value: '\$${_model.textController2.text}'),
                  _ConfirmRow(
                      label: 'Fecha', value: _model.textController3.text),
    ...
    try {
      ...
      final data = createReportepagoRecordData(
        ...
        comentario: _model.textController4.text.isNotEmpty
            ? _model.textController4.text
            : 'Sin comentario',
        refcliente: widget.refcliente,
        nombrecliente: widget.nombre,
      )..['propietarioUid'] = _uid; // para identificar pagos del dueño
      await ref.set(data);
      _model.pago = ReportepagoRecord.getDocumentFromData(data, ref);

      // 1b. Registrar el cobro automáticamente en la colección finanzas
      //     (para que Mis Finanzas e Informes → Financiero tengan el ingreso real).
      try {
        await FinanzasCobrosService.registrarCobro(
          uid: _uid,
          pagoId: ref.id,
          nombreCliente: widget.nombre ?? 'Cliente',
          monto:
              double.tryParse(_model.textController2.text.trim())?.toDouble() ??
                  0,
          fecha: DateTime.now(),
        );
      } catch (e) {
        debugPrint('[StarkGo] No se pudo registrar el cobro en finanzas: $e');
      }

      // 2. Actualizar estado → activo y guardar fecha del último pago
      await widget.refcliente!.update(createClientesRecordData(
          status: 'activo', ultimoPago: DateTime.now()));
    ...
    return Padding(
      ...
      child: Row(children: [
        ...
        Expanded(
          ...
          Text('Completa los datos del pago',
        ...
        Container(
          ...
          decoration: BoxDecoration(
              ...
              border:
                  Border.all(color: _C.whatsapp.withOpacity(0.3), width: 1)),
          child: Row(children: [
  ...
  Widget _buildClienteBanner() {
    final inicial = (widget.nombre?.isNotEmpty ?? false)
      ...
      child: Row(children: [
        ...
        Column(crossAxisAlignment: CrossAxisAlignment.end, children: [
          Text('Plan mensual',
              style: GoogleFonts.spaceGrotesk(
                  color: Colors.white54, fontSize: 11)),
          Text('\$$_planFormateado',
              style: GoogleFonts.spaceGrotesk(
                  color: _C.accent, fontSize: 18, fontWeight: FontWeight.w800)),
        ]),
      ]),
    );
  }
      ...
      child: Padding(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          _PagoField(
            ...
            validator: (val) =>
                _model.textController1Validator.asValidator(context)?.call(val),
          ),
          const SizedBox(height: 14),
          _PagoField(
                ...
                child: Container(
                    ...
                    child: Text('COP',
                        style: GoogleFonts.spaceGrotesk(
                            ...
                            fontWeight: FontWeight.w700)))),
            validator: (val) =>
                _model.textController2Validator.asValidator(context)?.call(val),
          ),
          const SizedBox(height: 14),
          _PagoField(
            ...
            validator: (val) =>
                _model.textController3Validator.asValidator(context)?.call(val),
          ),
          const SizedBox(height: 14),
          _PagoField(
            ...
            maxLines: 3,
            validator: (val) =>
                _model.textController4Validator.asValidator(context)?.call(val),
          ),
          const SizedBox(height: 14),
          _PagoField(
    ...
    return Container(
      ...
      child: Row(children: [
        ...
        Expanded(
          ...
          Text(
              'Al registrar, el estado cambia a Activo, se restablece el servicio en MikroTik y se envía comprobante por WhatsApp.',
              style:
                  GoogleFonts.spaceGrotesk(color: _C.textSec, fontSize: 11.5)),
        ])),
      ]),
    );
  }
                    ...
                    ? Row(mainAxisSize: MainAxisSize.min, children: [
                        SizedBox(
                            ...
                            child: CircularProgressIndicator(
                                strokeWidth: 2,
                                valueColor:
                                    AlwaysStoppedAnimation(_C.textSec))),
                        const SizedBox(width: 10),
                        Text('Registrando...',
                            style: GoogleFonts.spaceGrotesk(
                                color: _C.textSec,
  ...
  Widget build(BuildContext context) {
    return Padding(
      ...
      child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
        ...
        Text(value,
            style: GoogleFonts.spaceGrotesk(
                color: _C.textPri, fontSize: 13, fontWeight: FontWeight.w600)),
      ]),
    );
  }
}
