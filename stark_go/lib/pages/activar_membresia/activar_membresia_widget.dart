...
class ActivarMembresiaWidget extends StatefulWidget {
  const ActivarMembresiaWidget({super.key});

  static String routeName = 'ActivarMembresia';
  static String routePath = 'activarMembresia';

  @override
  State<ActivarMembresiaWidget> createState() => _ActivarMembresiaWidgetState();
}

class _ActivarMembresiaWidgetState extends State<ActivarMembresiaWidget> with TickerProviderStateMixin {
  Plan? _planSel;
          ...
          child: Column(children: [
            ...
            Expanded(
              child: SingleChildScrollView(
                ...
                child: Column(children: [
                  // ── Hero bienvenida ──
                  _buildHero().animate().fadeIn(duration: 400.ms).slideY(begin: 0.06, end: 0, duration: 400.ms, curve: Curves.easeOut),
                  ...
                  _buildNota().animate().fadeIn(duration: 400.ms, delay: 320.ms),
                ]),
              ),
            ),
          ]),
  ...
  // ── Top bar sin botón atrás (primera vez no puede volver) ──
  Widget _buildTopBar() {
    return Padding(
      ...
      child: Row(children: [
        ...
        Expanded(
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Text('Activar Membresía', style: GoogleFonts.dmSans(color: Colors.white, fontSize: 19, fontWeight: FontWeight.w700)),
            Text('StarkGo · Bienvenido', style: GoogleFonts.dmSans(color: Colors.white38, fontSize: 11.5)),
          ]),
        ),
        // Badge "Nuevo"
        Container(
          ...
          child: Row(mainAxisSize: MainAxisSize.min, children: [
            AnimatedBuilder(
              ...
              builder: (_, __) => Container(
                ...
                height: 7,
                decoration: BoxDecoration(
                  color: _C.accent.withOpacity(0.5 + _pulseController.value * 0.5),
  ...
  // ── Hero: bienvenida en vez de "acceso suspendido" ──
  Widget _buildHero() {
    return Container(
      width: double.infinity,
      decoration: BoxDecoration(
        gradient: const LinearGradient(
          colors: [Color(0xFF0C2340), Color(0xFF0F3460)],
          begin: Alignment.topLeft,
          end: Alignment.bottomRight,
        ),
        ...
        boxShadow: [
          BoxShadow(color: _C.primary.withOpacity(0.2), blurRadius: 24, offset: const Offset(0, 10)),
        ],
      ),
        ...
        Padding(
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Row(children: [
              ...
              Expanded(
                child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  Text('¡Bienvenido a StarkGo!', style: GoogleFonts.dmSans(color: Colors.white, fontSize: 16, fontWeight: FontWeight.w700)),
                  const SizedBox(height: 3),
                  Text(
                    _nombreUsuario.isNotEmpty ? 'Hola, $_nombreUsuario 👋' : 'Elige tu primer plan',
            ...
            Container(
              ...
              child: Row(children: [
                _heroStep('1', 'Registro', _C.success, Icons.check_circle_rounded),
                Container(width: 1, height: 40, color: Colors.white.withOpacity(0.08)),
                _heroStep('2', 'Elige plan', _C.primary, Icons.tune_rounded),
                Container(width: 1, height: 40, color: Colors.white.withOpacity(0.08)),
                _heroStep('3', 'Activa', _C.textSec, Icons.wifi_rounded),
              ]),
            ),
            const SizedBox(height: 14),
            // Nota informativa
            Container(
              ...
              child: Row(children: [
                ...
                Expanded(
                  child: Text(
                    'Selecciona el plan que mejor se adapte a ti y completa el pago para activar tu acceso.',
                    style: GoogleFonts.dmSans(color: _C.accent, fontSize: 11.5),
                  ),
                ),
              ]),
            ),
  ...
  Widget _heroStep(String num, String label, Color color, IconData icon) {
    return Expanded(
      child: Padding(
        ...
        child: Column(children: [
          ...
          Text(num, style: GoogleFonts.dmSans(color: color, fontSize: 12, fontWeight: FontWeight.w700)),
  ...
  // ── Sección planes (igual que RenovarMembresia) ──
  ...
  Widget _buildResumen() {
    ...
    return Container(
      decoration: BoxDecoration(
        ...
        boxShadow: [
          BoxShadow(color: _planSel!.color.withOpacity(0.1), blurRadius: 16, offset: const Offset(0, 6)),
        ],
      ),
      child: Column(children: [
        Container(
          ...
          child: Row(children: [
            ...
            const SizedBox(width: 8),
            Text('Resumen de activación', style: GoogleFonts.dmSans(color: Colors.white70, fontSize: 13, fontWeight: FontWeight.w600)),
            const Spacer(),
            Container(
        ...
        Padding(
          ...
          child: Column(children: [
            ...
            _resumenFila(Icons.event_rounded, 'Vence el', _formatFecha(fecha), _C.accent),
            _resumenFila(Icons.calendar_month_rounded, 'Días de acceso', '$dias días', _C.success),
  ...
  Widget _buildNota() {
    return Container(
      ...
      child: Row(children: [
        ...
        Expanded(
          child: Text(
            'Al completar el pago, tu acceso se activa inmediatamente y recibirás confirmación por WhatsApp.',
...
// ═══════════════════════════════════════
//  PLAN CARD (idéntica a RenovarMembresia)
// ═══════════════════════════════════════
...
class _FondoPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    drawCircle(-40, size.height * 0.15 + math.sin(t * 2 * math.pi) * 20, 180, const Color(0xFF1A73E8), 0.05);
    drawCircle(size.width + 40, size.height * 0.55 + math.cos(t * 2 * math.pi) * 15, 150, const Color(0xFF00C6AE), 0.04);
    drawCircle(size.width * 0.5, size.height * 0.85, 100, const Color(0xFF7C3AED), 0.03);
